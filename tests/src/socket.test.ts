...
const game = defineGame({ id: 'test-game', entities: { racer: Racer }, player: 'racer' });
...
/** Poll until the server has caught up, rather than guessing at a delay. */
...
test('client and server talk over the socket', async t => {
	...
	await t.test('the joining client gets the world before the first tick', async () => {
		...
		assert.equal(world.entities[0].$, 'test-game:racer');
		assert.equal(world.entities[0].id, entityId);
	});

	const entity = () => [...server.world.entities.values()][0];

	await t.test('input reaches the component that handles it', async () => {
		assert.equal(entity().get(Runner).distance, 0);

		client.emit('input', 'run', true);
		await waitFor('the server to see the action', () => entity().get(Runner).running);

		for (let i = 0; i < 5; i++) server.world.tick();

		assert.equal(entity().get(Runner).distance, 5, 'five ticks while running should cover five units');
	});

	await t.test('releasing the action stops it', async () => {
		client.emit('input', 'run', false);
		await waitFor('the server to see the release', () => !entity().get(Runner).running);

		for (let i = 0; i < 5; i++) server.world.tick();

		assert.equal(entity().get(Runner).distance, 5);
	});
