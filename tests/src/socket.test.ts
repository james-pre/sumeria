import { Component, Entity, player, playerType, type EntitySaveData, type WorldData } from '@sumeria/core';
...
test('the decorated class becomes the player type', () => {
	assert.equal(playerType, Racer);
});
...
test('client and server talk over the socket', async t => {
	...
	const welcomed = next<{ entity: string | null; world: WorldData }>(client, 'welcome');
	...
	await t.test('a connecting client is welcomed with a player entity', async () => {
		...
		assert.equal(welcome.world.entities.length, 1);
		assert.equal(welcome.world.entities[0].$, 'Racer');
		entityId = welcome.entity;
	});

	const runner = () => [...server.world.entities.values()][0].get(Runner)!;

	await t.test('the first tick reports the player as added', () => {
		const diff = server.world.tick();
	...
	await t.test('ticks carry component state to the client', async () => {
		const received = next<{ updated: (EntitySaveData & RunnerData)[] }>(client, 'tick');

		server.world.tick();

		const diff = await received;
		assert.equal(diff.updated[0].distance, 5);
	});
	...
	await t.test('a disconnecting client takes its player entity with it', async () => {
		...
		assert.deepEqual(server.world.tick().removed, [entityId]);
	});
});
