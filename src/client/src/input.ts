import { triggerOf, type ActionBinding, type Controls } from '@sumeria/core';
...
function onKey(event: KeyboardEvent, down: boolean): void {
	...
	for (const [action, binding] of bindings) {
		...
		// Matched on the key alone: letting go of a modifier first would otherwise leave the action stuck on.
